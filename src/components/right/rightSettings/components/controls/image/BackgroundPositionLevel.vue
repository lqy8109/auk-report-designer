 ...
 * @LastEditors: xwj
 * @LastEditTime: 2026-02-04 15:57:53
 * @description: 
-->
<script setup>
...
const value = ref(props.form["background-position"]?.split(" ")[0] || "left");

watch(
  () => props.form["background-position"],
  (val) => {
    value.value = props.form["background-position"]?.split(" ")[0] || "left";
  }
);
...
const handleChange = (val) => {
  ...
  if (backgroundPosition) {
    var level = backgroundPosition.split(" ")[1];

    var style = selectedComp.value.style || {};
    const newStyle = {
      ...style,
    };
    newStyle[props.type] = `${val} ${level}`;
